        ...
        <tbody>
            <?php foreach($listados AS $listado){ ?>
                <tr id="tr-<?= $listado["id"]; ?>">
                    ...
                    <td class="vM">
                        <?php
                            $semana = $_TUCOACH->get_data("olc_semanas", " AND id = ".$listado["id_semana"]." AND eliminado = 0 ORDER BY id DESC ", 0);
                            if($semana) echo ($semana["semana"]."<br><small>".$semana["cargo"]."</small>");
                        ?>
                    </td>
                    <td class="vM">
                        <?php
