        ...
        <tbody>
            <?php foreach($listados AS $listado){ ?>
                <tr id="tr-<?= $listado["id"]; ?>">
                    ...
                    <td class="vM taC">
                        <?php
                            if($listado["visible"] == 1) echo '<i class="la la-check success t24"></i>';
                            if($listado["visible"] == 0) echo '<i class="la la-close danger t24"></i>';
                        ?>
                    </td>
                    <td class="vM taC" style="padding:0 !important;">
                        <?php
                            if($listado["imagen"] != ""){
                                echo '<img src="'.$rutaStatic.'articulos/s/'.($listado["imagen"]).'" class="w100x" />';
                            } else echo '<img src="'.$sinImagen.'" class="w100x" />';
                        ?>
                    </td>
                    <td class="vM"><?= ($listado["nombre"]); ?></td>
                    <td class="vM"><?= ($listado["resumen"]); ?></td>
