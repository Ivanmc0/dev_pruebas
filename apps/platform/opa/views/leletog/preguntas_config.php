...
if ($access_model["tipo"] == 2) {
    ...
    $edit       = true;
    $getModelo  = $_ZOOM->get_data('grw_lel_preguntas', " AND id = " . $getThis['id'] . " ORDER BY id DESC ", 0);
    $getMod     = $_ZOOM->get_data('grw_lel_dinamicas', " AND id = " . $getModelo['id_dinamica'] . " ORDER BY id DESC ", 0);
    $tipo       = $getMod['id_tipo'];
} else {
    $getMod         = $_ZOOM->get_data('grw_lel_dinamicas', " AND id = " . $id . " ORDER BY id DESC ", 0);
    $tipo           = $getMod['id_tipo'];
}
...
?>
    ...
    <div class="content-body">
        <form action="modulos/accion-all" id="formion" name="formion" method="post" class="form-horizontal zoom_form">
            ...
            <?php } else { ?>
                <input type="hidden" id="id_dinamica" name="id_dinamica" value="<?= $id; ?>" />
            <?php } ?>
                        ...
                        <div class="row mb20">
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-control form-group-margin" id="id_modo" name="id_modo">
                                    ...
                                    <?php
                                    $getModModelo = $_ZOOM->get_data("olc_preguntas_tipos", " AND id_tipo = " . $tipo . " AND inactivo = 0 AND eliminado = 0 ORDER BY id ASC ", 1);
                                    if ($getModModelo) {
                                        foreach ($getModModelo as $getModeloMod) {
                                    ?>
                                            <option value="<?= ($getModeloMod["id"]); ?>" <?php if (($edit && $getThis["id_modo"] == $getModeloMod["id"])) echo "selected"; ?>><?= ($getModeloMod["nombre"]); ?></option>
                                    <?php
                                        }
                                    }
                                    ?>
