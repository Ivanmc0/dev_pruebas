                    ...
                    <div class="row mb20">
                        ...
                        <div class="col-md-3">
                            ...
                            <select class="form-control form-group-margin" id="id_categoria" name="id_categoria" >
                                ...
                                <?php
                                    if($edit)   $estaes = $_TUCOACH->get_data("grw_tuc_p2p_categorias", " AND id = ".$getThis["id_categoria"]." AND inactivo = 0 AND eliminado = 0 ORDER BY id ASC ", 0);
                                    else        $estaes = $_TUCOACH->get_data("grw_tuc_p2p_categorias", " AND id = $id AND inactivo = 0 AND eliminado = 0 ORDER BY id ASC ", 0);
                                    $getCategorias = $_TUCOACH->get_data("grw_tuc_p2p_categorias", " AND id_perfil = ".$estaes["id_perfil"]." AND inactivo = 0 AND eliminado = 0 ORDER BY id ASC ", 1);
                                    if($getCategorias){
                                        foreach($getCategorias AS $getCategoria){
                                ?>
                                            <option value="<?= ($getCategoria["id"]); ?>" <?php if(($edit && $getThis["id_categoria"] == $getCategoria["id"]) || ($edit == false && $id == $getCategoria["id"])) echo "selected"; ?>><?= ($getCategoria["nombre"]); ?></option>
                                <?php
                                        }
                                    }
                                ?>
