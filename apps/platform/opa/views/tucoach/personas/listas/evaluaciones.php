        ...
        <tbody>
            <?php foreach($listados AS $listado){ ?>
                <tr id="tr-<?= $listado["id"]; ?>">
                    ...
                    <td class="vM">
                        <?= ($listado["nombre"]); ?>
                        <!-- <br><small><?= ($listado["descripcion"]); ?></small> -->
                    </td>
                    <td class="vM">
                        <?php
                    ...
                    <td class="vM">
                        <?php
                            $test = $_TUCOACH->get_data("grw_tuc_p2p_tests", " AND id = ".$listado["id_test"]." AND eliminado = 0 ORDER BY id DESC ", 0);
                            if($test) echo ($test["nombre"]);
                        ?>
                    </td>
                    <td class="vM">
                        <?php
                            $gruporoles = $_TUCOACH->get_data("grw_tuc_roles_grupos", " AND id = ".$listado["id_gruporol"]." AND eliminado = 0 ORDER BY id DESC ", 0);
