            ...
            <?php foreach($listados AS $listado){ ?>
                <tr id="tr-<?= $listado["id"]; ?>">
                    ...
                    <td class="vM" style="padding:10px !important;">
                        <?php
                            $categorias = $_TUCOACH->get_data("grw_tuc_p2p_categorias", " AND id_perfil = ".$listado["id"]." AND eliminado = 0 ORDER BY id DESC ", 1);
                            if($categorias) {
                                foreach($categorias AS $categoria){
                                    echo '
                                        <div class="tab bS1 mb-1">
                                            <div class="tabIn p510">'.($categoria["nombre"]).'</div>
                                        </div>
                                    ';
                                }
                            } else echo "Sin datos";
