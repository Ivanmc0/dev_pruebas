                    ...
                    <div class="row mb20">
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-control form-group-margin" id="id_test" name="id_test" >
                                ...
                                <?php
                                    if($edit)   $estaes = $_TUCOACH->get_data("grw_tuc_p2p_tests", " AND id = ".$getThis["id_test"]." AND inactivo = 0 AND eliminado = 0 ORDER BY id ASC ", 0);
                                    else        $estaes = $_TUCOACH->get_data("grw_tuc_p2p_tests", " AND id = $id AND inactivo = 0 AND eliminado = 0 ORDER BY id ASC ", 0);
                                    $getTests = $_TUCOACH->get_data("grw_tuc_p2p_tests", " AND id = ".$estaes["id"]." AND inactivo = 0 AND eliminado = 0 ORDER BY id ASC ", 1);
                                    if($getTests){
                                        foreach($getTests AS $getTest){
                                ?>
                                            <option value="<?= ($getTest["id"]); ?>" <?php if(($edit && $getThis["id_test"] == $getTest["id"]) || ($edit == false && $id == $getTest["id"])) echo "selected"; ?>><?= ($getTest["nombre"]); ?></option>
                                <?php
                                        }
                                    }
                                ?>
