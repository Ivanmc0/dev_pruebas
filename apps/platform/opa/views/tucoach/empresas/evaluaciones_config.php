            ...
            <div class="card-content collapse show">
                ...
                <div class="card-body">
                    ...
                    <div class="row mb20">
                        ...
                        <div class="col-md-4">
                            <label class="control-label">Grupo Tests</label>
                            <select class="form-control form-group-margin" id="id_grupotests" name="id_grupotests">
                                <option value="0">Seleccione</option>
                                <?php
                                    $getTests = $_TUCOACH->get_data("grw_tuc_paquetests", " AND inactivo = 0 AND eliminado = 0 ORDER BY id ASC ", 1);
                                    if($getTests){
                                        foreach($getTests AS $getTest){
                                ?>
                                            <option value="<?= ($getTest["id"]); ?>" <?php if($edit && $getThis["id_grupotests"] == $getTest["id"]) echo "selected"; ?>><?= ($getTest["nombre"]); ?></option>
                                <?php
                                        }
                                    }
                                ?>
                        ...
                        <div class="col-md-4">
                            <label class="control-label">Grupo de segmentación</label>
                            <select class="form-control form-group-margin" id="id_segmentos" name="id_segmentos">
                                <option value="0">Seleccione</option>
                                <?php
                                    $getSegmentos = $_TUCOACH->get_data("grw_tuc_segmentaciones_grupo", " AND inactivo = 0 AND eliminado = 0 ORDER BY id ASC ", 1);
                                    if($getSegmentos){
                                        foreach($getSegmentos AS $getSegmento){
                                ?>
                                            <option value="<?= ($getSegmento["id"]); ?>" <?php if($edit && $getThis["id_segmentos"] == $getSegmento["id"]) echo "selected"; ?>><?= ($getSegmento["nombre"]); ?></option>
                                <?php
                                        }
                                    }
                                ?>
