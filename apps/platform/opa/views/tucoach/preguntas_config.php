                ...
                <div class="card-body">
                    ...
                    <div class="row mb20">
                        ...
                        <div class="col-md-2">
                            ...
                            <input class="form-control form-group-margin" type="text" id="desde" name="desde" value="<?php if($edit) echo ($getThis["desde"]); ?>" />
                        </div>
                        <div class="col-md-2">
                            <label class="control-label">Hasta</label>
                            <input class="form-control form-group-margin" type="text" id="hasta" name="hasta" value="<?php if($edit) echo ($getThis["hasta"]); ?>" />
                        </div>
                        <div class="col-md-3">
                            <label class="control-label">Grupo de preguntas</label>
                            <select class="form-control form-group-margin" id="id_grupopregunta" name="id_grupopregunta">
                                <option value="0">Ninguno</option>
                                <?php
                                    $getGrupos = $_TUCOACH->get_data("grw_paquete_respuestas", " AND inactivo = 0 AND eliminado = 0 ORDER BY id ASC ", 1);
                                    if($getGrupos){
                                        foreach($getGrupos AS $getGrupo){
                                ?>
                                            <option value="<?= ($getGrupo["id"]); ?>" <?php if(($edit && $getThis["id_grupopregunta"] == $getGrupo["id"]) || ($edit == false && $id == $getGrupo["id"])) echo "selected"; ?>><?= ($getGrupo["nombre"]); ?></option>
                                <?php
                                        }
                                    }
                                ?>
