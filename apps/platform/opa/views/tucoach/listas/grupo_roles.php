            ...
            <?php foreach($listados AS $listado){ ?>
                <tr id="tr-<?= $listado["id"]; ?>">
                    ...
                    <td class="vM" style="padding:10px !important;">
                        <?php
                            $contenidos = $_TUCOACH->get_data("grw_tuc_roles", " AND id_gruporol = ".$listado["id"]." AND eliminado = 0 ORDER BY id DESC ", 1);
                            if($contenidos) {
                                foreach($contenidos AS $contenido){
                                    echo '
                                        <div class="tab bS1 mb-1">
                                            <div class="tabIn p510">'.($contenido["nombre"]).'</div>
                                            <div class="tabIn p510 w30x"><div class="wh20 rr50 mAUTO" style="background:'.($contenido["color"]).'"></div></div>
                                        </div>
                                    ';
                                }
                            } else echo "Sin datos";
