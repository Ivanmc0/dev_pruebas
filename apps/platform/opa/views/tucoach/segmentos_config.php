            ...
            <div class="card-content collapse show">
                ...
                <div class="card-body">
                    ...
                    <div class="row mb20">
                        ...
                        <div class="col-md-4">
                            <label class="control-label">Grupo de segmentos</label>
                            <select class="form-control form-group-margin" id="id_gruposegmento" name="id_gruposegmento">
                                <option value="0">Ninguno</option>
                                <?php
                                    $getGrupos = $_TUCOACH->get_data("grw_tuc_segmentaciones_grupo", " AND inactivo = 0 AND eliminado = 0 ORDER BY id ASC ", 1);
                                    if($getGrupos){
                                        foreach($getGrupos AS $getGrupo){
                                ?>
                                            <option value="<?= ($getGrupo["id"]); ?>" <?php if(($edit && $getThis["id_gruposegmento"] == $getGrupo["id"]) || ($edit == false && $id == $getGrupo["id"])) echo "selected"; ?>><?= ($getGrupo["nombre"]); ?></option>
                                <?php
                                        }
                                    }
                                ?>
