<?php require_once('../../../appInit.php');
    ...
    $thisCargo = $_LELE->ZOOM->get_data("grw_cargos", " AND id = " . $plana[$data["trabajador"]]["cargo_id"]." AND id_empresa = " . $_SESSION["lele_empresa_id"] . " AND eliminado = 0 ORDER BY id ASC ", 0);
    if($thisCargo){
        $cargosNiveles = $_LELE->ZOOM->get_data("grw_cargos", " AND nivel = " . $thisCargo["nivel"] . " AND id_organigrama = " . $thisCargo["id_organigrama"] . " AND id_empresa = " . $_SESSION["lele_empresa_id"] . " AND eliminado = 0 ORDER BY id ASC ", 1);
        if($cargosNiveles){
            foreach ($cargosNiveles as $key => $cargosNivel) {
                //$selCargos = $_LELE->ZOOM->get_data("___olc_trabajadores_carg0", " AND id_cargo = " . $cargosNivel["id"] . " AND id_empresa = " . $_SESSION["lele_empresa_id"] . " AND eliminado = 0 ORDER BY id ASC ", 1);
                $selCargos = $_WORKERS->getCargo(" AND id_cargo = " . $cargosNivel["id"] . " AND id_empresa = " . $_SESSION["COMPANY"]["id"] . " AND eliminado = 0"  );
                if($selCargos){
                    foreach ($selCargos as $key => $tru) {
                        ...
                        $name = $_LELE->ZOOM->get_data("zoom_users", " AND id = " . $tru["id_trabajador"] . " AND eliminado = 0 ORDER BY id ASC ", 0);
    ...
    foreach ($plana as $key => $trab) {
        $respuesta = $_LELE->ZOOM->get_data("grw_sol_act_encuestas", " AND id_actividad = " . $data["actividad"] . " AND id_trabajador = " . $trab["id"] . " AND id_empresa = " . $_SESSION["lele_empresa_id"] . " AND eliminado = 0 ORDER BY id DESC ", 0);
        if($respuesta){
    ...
    echo '
        <div class="row align-items-center mb30" style="z-index:5">
            ...
            <div class="col-12 col-lg-6">
                ...
                <select class="w100 dB p1020 bS1 rr5" onchange="lele.lider('.$data["empresa"].', this.value, 0, '.$data["actividad"].');">
    ';
                $yo = 0;
                foreach ($trus as $key => $tru) {
                    if($tru["id"] != $_SESSION["lele_id"] && $yo == 0){
                        echo '<option value="'.$_SESSION["lele_id"].'">YO</option>';
                    }
                    if($tru["lider"]){
    ...
    echo '
        <div class="tab bVerde2 bS1 bCVerde colorVerde ff3 p20" style="z-index:5">
            <div class="tabIn">COLABORADOR</div>
    ...
    foreach ($plana as $key => $trab) {
        ...
        echo '
            <div class="tab bS1 p1020" style="margin-top:-1px;">
                <div class="tabIn">
        ...
        if($yes){
            echo '
                <div class="t10 aS ff0 cP" data-toggle="modal" data-target="#modalDetalleSolucion" onClick="lele.actividad_detalle('.$data["actividad"].', '.$trab["id"].', '.$data["empresa"].')">
                    Detalle
                </div>
            ';
        }
