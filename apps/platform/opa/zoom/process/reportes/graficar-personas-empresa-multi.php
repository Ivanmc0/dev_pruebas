<?php require_once ('../../../appInit.php');
    ...
    if($thisEvaluacion){
        ...
        if($_POST["cond2"] == ""){
            ...
            $ross[0] = $_TUCOACH->get_data( $complement.$cond." AND EMP.id = {$thisEvaluacion["id_empresa"]} AND ASIG.id_evaluacion = {$thisEvaluacion["id"]} ORDER BY CAT.id DESC ", $complement2.$cond2." AND EMP.id = {$thisEvaluacion["id_empresa"]} AND ASIG.id_evaluacion = {$thisEvaluacion["id"]} ");
            ...
            foreach (unserialize($_POST["cond2"]) as $key => $segs) {
                ...
                $ross[] = $_TUCOACH->get_data( $complement.$cond." AND EMP.id = {$thisEvaluacion["id_empresa"]} AND ASIG.id_evaluacion = {$thisEvaluacion["id"]} ORDER BY CAT.id DESC ", $complement2." AND EMP.id = {$thisEvaluacion["id_empresa"]} AND ASIG.id_evaluacion = {$thisEvaluacion["id"]} ");
            }
