<?php
    if($mud = $_PLATFORM->PermissionValidationModel ($app, $geton[3], true)){
        if($registro = $_ZOOM->get_data("grw_val_asignaciones", " AND uuid = '".$geton[4]."' AND id_empresa = ".$_SESSION['COMPANY']['id']." AND eliminado = 0 ", 0)){
?>
...
<?php
    $tabla = $registro['id_trabajador'] == "" ? 'grw_val_listasexternas_registros' : 'grw_val_listasinternas_registros';
	$campo = $registro['id_trabajador'] == "" ? 'id' : 'id_trabajador';
	$value = $registro['id_trabajador'] == "" ? $registro['id'] : $registro['id_trabajador'];

	if($dato = $_TUCOACH->get_data($tabla, " AND $campo = $value AND eliminado = 0 ORDER BY id DESC ", 0)){

		if($registro['id_trabajador'] != "" && $people = $_ZOOM->get_data("zoom_users", " AND id = ".$registro['id_trabajador']." ", 0)){
			$dato["nombre"] = $people["nombre"];
			$dato["email"] = $people["email"];
			$dato["celular"] = $people["celular"];
			$dato["empresa"] = 'Banco W';
			$dato["cargo"] = $people["id_cargo"];
		}
...
?>
		...
		<?php
			...
			if($respuestas = $_TUCOACH->get_data("grw_sol_val_listaexterna", " AND id_asignacion = '".$registro['id']."' AND eliminado = 0 ORDER BY id DESC ", 1)){
				$arrRespuestas = [];
			...
			<div class="bfff bShadow3 rr20 p30 p20_oS mb50">
				...
				<div class="row">
					<?php
						...
						$faq    = 1;
						$bloque = 1;
						if($encPregs = $_TUCOACH->get_data("grw_val_preguntas", " AND id_encuesta = '".$registro['id_encuesta']."' AND eliminado = 0 ORDER BY id ASC ", 1)){
