...
?>
        ...
        <div class="bfff p40 p20_oS bBS1 mb20">
            ...
            <?php
                //$jefe = $_ZOOM->get_data('___olc_trabajadores_carg0', ' AND id_jefe = ' . $_SESSION["WORKER"]["id"] . ' AND inactivo = 0 AND eliminado = 0', 1);
                $jefe = $_ZOOM->$_WORKERS->getCargo( ' AND id_jefe = ' . $_SESSION["WORKER"]["id"] . ' AND inactivo = 0 AND eliminado = 0', 1);
                if($jefe){
            ?>
                <a href="<?= $dominion."lider/".$geton[2]."/".$geton[3]."/"; ?>" class="dIB fR bVerde taC colorfff rr5 cP bHover bS1 bCVerde">
        ...
        <div class="general pAA30">
            ...
            <script>
                $( document ).ready(function() {
                    lele.actividad_detalle(<?= $actividad["id"]; ?>, <?= $_SESSION["WORKER"]["id"]; ?>, <?= $_SESSION["COMPANY"]["id"]; ?>);
                });
            </script>
