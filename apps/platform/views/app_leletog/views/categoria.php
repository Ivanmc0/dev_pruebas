<?php
    if(isset($geton[3])) {
        $categoria      = $_ZOOM->get_data('grw_lel_categorias', ' AND id = ' . $geton[3] . ' AND inactivo = 0 AND eliminado = 0', 0);
        $actividades    = $_ZOOM->get_data('grw_lel_actividades', ' AND id_categoria = ' . $geton[3] . ' AND id_empresa = '.$_SESSION["COMPANY"]["id"].' AND inactivo = 0 AND eliminado = 0', 1);
        if($actividades){
?>
                ...
                <?php
                    foreach ($actividades as $act) {
                        if($act["tipo"] == 1 || ($act["tipo"] == 2 && $_LELE->busquedaEnLista($act["asignados"], $_SESSION["WORKER"]["id"]))){
                            ...
                            if($balance){
                                if($balance["status_1"] < $balance["inter"]) echo '<a href="'.$dominion.'actividad/'.$_ZOOM->url_seo($act['nombre']).'/'.$act['id'].'/">';
                                else                                         echo '<a href="'.$dominion.'detalle/'.$_ZOOM->url_seo($act['nombre']).'/'.$act['id'].'/">';
                ?>
