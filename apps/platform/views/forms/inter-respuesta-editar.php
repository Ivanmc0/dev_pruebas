<?php
    $edit = 0;
    if($edit = $_ZOOM->get_data( "grw_lel_respuestas", " AND uuid = '".$_MOD['gist']."' AND eliminado = 0 ", 0)){
        $id_dinamica = $_ZOOM->get_data( "grw_lel_preguntas", " AND id = '".$edit['id_pregunta']."' AND eliminado = 0 ", 0)["id_dinamica"];
        ...
        <div class="col-12 col-lg-12 p0 m0 p0_oS">
            <div class="pAA10">
                ...
                <select class="select" name="correcta">
                    <option <?php if($edit['correcta'] == 0) echo 'selected'; ?> value="0">No</option>
                    <option <?php if($edit['correcta'] == 1) echo 'selected'; ?> value="1">Sí</option>
                </select>
            </div>
        </div>
    </div>
