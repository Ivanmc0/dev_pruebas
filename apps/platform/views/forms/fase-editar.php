<?php
    $edit = 0;
    if($edit = $_ZOOM->get_data( "grw_val_fases", " AND uuid = '".$_MOD['gist']."' AND eliminado = 0 ", 0)){
?>
    ...
    <div class="col-12 col-lg-4 p0 m0 pL5 p0_oS">
        <div class="pAA10">
            ...
            <select class="select" name="id_etapa">
                ...
                <option <?php if($edit['id_etapa'] == 1) echo 'selected'; ?> value="1">Antes</option>
                <option <?php if($edit['id_etapa'] == 2) echo 'selected'; ?> value="2">Durante</option>
                <option <?php if($edit['id_etapa'] == 3) echo 'selected'; ?> value="3">Después</option>
            </select>
        </div>
    </div>
</div>
