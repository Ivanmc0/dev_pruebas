...
<?php $thiss 		= $_ZOOM->get_data("grw_okr_sprints", " AND id = ".$geton[2]." AND inactivo = 0 AND eliminado = 0 ", 0); ?>
<?php if($thiss){ ?>

<div class="max700 pAA40 mAUTO">

	<div class="p20 t20 ff3 ff0 colorfff bMorado">
		<small class="ff1 coloreee">Editando el Sprint Mensual</small><br><?= ($thiss["nombre"]); ?>
	</div>
	<div class="p30">
				...
				<div class="col-12 col-lg-6">
					...
					<select class="p1020 w100 bS1 rr5 dB mb10" name="ano" id="ano">
						<?php for($ff=2024; $ff<=2025; $ff++){ ?>
							<option <?php if($thiss["ano"] == $ff) echo "selected"; ?> value="<?= $ff; ?>"><?= $ff; ?></option>
						<?php } ?>
					</select>
				</div>
				<div class="col-12 col-lg-6">
					...
					<select class="p1020 w100 bS1 rr5 dB mb10" name="mes" id="mes">
						<?php for($ff=1; $ff<=12; $ff++){ ?>
							<option <?php if($thiss["mes"] == $ff) echo "selected"; ?> value="<?= $ff; ?>"><?= $_ZOOM->verMes($ff); ?></option>
						<?php } ?>
					</select>
				</div>
			</div>
