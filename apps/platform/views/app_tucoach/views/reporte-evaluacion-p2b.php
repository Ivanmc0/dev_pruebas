<div class="p30">
        ...
        <div class="pAA20">
            ...
            ?>
                        <div class="row bBS1 align-items-center">
                            <div class="col-12 col-md-7 col-lg-8 p30 ff2">
                                <?= ($compota["comportamiento"]); ?>
                            </div>
                            <?php
                                $respuesta   = $_TUCOACH->get_data("grw_sol_p2b_estudio", " AND id_comportamiento = ".$compota['id_comportamiento']." AND id_asignacion = ".$thisAsignacion["id"]." AND inactivo = 0 AND eliminado = 0 ORDER by id DESC ", 0);
                                $pregunta1    = $_TUCOACH->get_data("grw_paquete_preguntas", " AND desde <= ".$respuesta["solucion"]." AND hasta >= ".$respuesta["solucion"]." AND id_grupopregunta = ".$resps['id']." AND inactivo = 0 AND eliminado = 0 ORDER by id DESC ", 0);
                                $pregunta2    = $_TUCOACH->get_data("grw_paquete_preguntas", " AND desde <= ".$respuesta["solucion2"]." AND hasta >= ".$respuesta["solucion2"]." AND id_grupopregunta = ".$resps['id']." AND inactivo = 0 AND eliminado = 0 ORDER by id DESC ", 0);
                            ?>
                            <div class="col-12 col-md-5 col-lg-4 mb20_oS">
                                <div class="form-group">
                                    ...
                                    <input type="range"
                                        ...
                                        step="<?= ($resps["saltos"]); ?>"
                                        value="<?= ($respuesta["solucion"]); ?>"
                                        min="<?= ($resps["inicio"]); ?>"
                                        max="<?= ($resps["fin"]); ?>"
                                        onchange="Ion.getCriterio(this.value, <?= $compota['id_comportamiento']; ?>, <?= $resps['id']; ?>);">
                                    <div id="rtn-resp-<?= $compota["id_comportamiento"]; ?>" class="taC mt5"><?= ($respuesta["solucion"]); ?> - <?= ($pregunta1["nombre"]); ?></div>
                                </div>
                            </div>
                            <div class="col-12 col-md-4 col-lg-3 mb20_oS dN"> <!-- OCULTO -->
                                ...
                                <div class="dN_oPC ff3 mb5">En el Proceso</div>
                                    <input type="range"
                                        ...
                                        class="form-control-range" id="resp-<?= $compota["id_comportamiento"]; ?>"
                                        step="<?= ($resps["saltos"]); ?>"
                                        value="<?= ($respuesta["solucion2"]); ?>"
                                        min="<?= ($resps["inicio"]); ?>"
                                        max="<?= ($resps["fin"]); ?>"
                                        onchange="Ion.getCriterio2(this.value, <?= $compota['id_comportamiento']; ?>, <?= $resps['id']; ?>);">
                                    <div id="rtn-resp2-<?= $compota["id_comportamiento"]; ?>" class="taC mt5"><?= ($respuesta["solucion2"]); ?> - <?= ($pregunta2["nombre"]); ?></div>
                                </div>
                            </div>
                        </div>
            <?php
    ...
    <?php
        ...
        } else echo "Esta evaluación no ha sido realizada";
