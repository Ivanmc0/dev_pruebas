<div class="p50 p30_oS">
    ...
    <h5 class="ff0 tU t12 mb10">Reporte de encuesta</h5>
    <h1 class="ff3 cPrimary t30 mb20"><?= ($encuesta["nombre"]); ?></h1>
    <h3 class="ff1 color666 t16 mb50"><?= ($encuesta["descripcion"]); ?></h3>
                        ...
                        <div class="card-content collapse show">
                            ...
                            <div class="card-body">
                                ...
                                <div class="">
                                    <?php
                                        $consol = [];
                                        $getPreguntas = $_ZOOM->get_data('grw_val_preguntas', ' AND id_encuesta = '.$listado["id"].' AND inactivo = 0 AND eliminado = 0', 1);
                                        if ($getPreguntas) {
                                            foreach ($getPreguntas as $numpre => $pre) {

                                                $respuestas = $_ZOOM->obtenerRespuestasEncuestaAnonima($pre["id"], 2, $pre["id_modo"], 0);
                                                    ...
                                                    if($respuestas){
                                                        foreach($respuestas["respuestas"] AS $key => $resp){
                                                            ...
                                                            if( $respuestas["soluciones"][$key] > 0){
                                                                if($pre["id"] != 1){
                                                                    $consol[$pre["id"]][$resp["id"]]["nombre"] = $resp["nombre"];
                                                                    ...
                                                                    if($resp["nombre"] >= 9) $nps["suman"] += $respuestas["soluciones"][$key];
                                                                    if($resp["nombre"] >= 7 && $resp["nombre"] <= 8) $nps["omitidos"] += $respuestas["soluciones"][$key];
                                                                    if($resp["nombre"] <= 6) $nps["restan"] += $respuestas["soluciones"][$key];

                                                                }
                                                            }

                                                            echo '<div class="dIB w10 p5">';
                                                    ...
                                                    if($pre["id"] != 1){
                                                        ...
                                                        if(isset($consol[$pre["id"]])){
                                                            foreach ($consol[$pre["id"]] as $l => $l2) {
                                                                $oo["votos"] += $l2["conteo"];
                                                                $oo["sumatoria"] += $l2["nombre"]*$l2["conteo"];
                                                            }
                                                        }
