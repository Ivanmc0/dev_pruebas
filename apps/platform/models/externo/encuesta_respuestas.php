...
$encuesta = $_ZOOM->get_data('grw_lel_dinamicas', ' AND id = ' . $_POST["enc"] . ' AND inactivo = 0 AND eliminado = 0', 0);
...
if($encuesta){
    ...
    if ($encuesta['id_modelo'] == 1) {
        if ($encuesta['id_tipo'] == 1) $material = ' evaluativo';
        else if ($encuesta['id_tipo'] == 2) $material = ' investigativo';
    }
    ...
    ';
            ...
            $getPreguntas = $_ZOOM->get_data('grw_lel_preguntas', ' AND id_dinamica = ' . $encuesta['id'] . ' AND inactivo = 0 AND eliminado = 0', 1);
            if ($getPreguntas) {
                        ...
                        if ($encuesta['id_tipo'] == 1) {
                            ...
                            if ($getRespuestas) {
                                ...
                                foreach ($getRespuestas as $res) {
                                    if ($pre['id_modo'] == 1) {
                                        $checkion = "bfff";
                                        if (isset($data_respuestas[$encuesta["id"] . "_" . $pre["id"]]) && $res["id"] == $data_respuestas[$encuesta["id"] . "_" . $pre["id"]]['id_respuesta']){
                                        ...
                                        $checkion = "bfff";
                                        if (!empty($data_respuestas[$encuesta["id"] . "_" . $pre["id"]]['respuesta_multiple'])) {
                                            foreach ($data_respuestas[$encuesta["id"] . "_" . $pre["id"]]['respuesta_multiple'] as $value) {
                                                if ($res['id'] == $value){
                                                    if($res["correcta"]) $respuestas_correctas++;
                                                    $checkion =  'bVerde colorfff';
                                                }
                            ...
                            if ($pre['id_modo'] != 5) {
                                ...
                                if ($getRespuestas) {
                                    foreach ($getRespuestas as $res) {
                                        if ($pre['id_modo'] == 1 || $pre['id_modo'] == 3) {
                                            $checkion = "bfff";
                                            if (isset($data_respuestas[$encuesta["id"] . "_" . $pre["id"]]) && $res["id"] == $data_respuestas[$encuesta["id"] . "_" . $pre["id"]]['id_respuesta']) $checkion = 'bVerde colorfff';
                                ...
                                $value = '';
                                if (isset($data_respuestas[$encuesta["id"] . "_" . $pre["id"]])) $value = ($data_respuestas[$encuesta["id"] . "_" . $pre["id"]]['respuesta']);
