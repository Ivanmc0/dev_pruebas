<?php require_once ('../../appInit.php');
	...
	function validarPerfil($id, $dato){
		...
		$existe = $_TUCOACH->get_data("grw_perfiles", " AND id_test = ".$id." AND nombre = '".($dato)."' ORDER BY id DESC ", 0);
		if($existe) {
	...
	function validarCategoria($id, $dato){
		require_once ('../../appInit.php');
		$existe = $_TUCOACH->get_data("grw_tuc_p2p_categorias", " AND id_perfil = ".$id." AND nombre = '".($dato)."' ORDER BY id DESC ", 0);
		if($existe) {
	...
	function validarCompetencia($id, $dato, $dato2){
		require_once ('../../appInit.php');
		$existe = $_TUCOACH->get_data("grw_tuc_p2p_competencias", " AND id_categoria = ".$id." AND nombre = '".($dato)."' ORDER BY id DESC ", 0);
		if($existe) {
	...
	function validarComportamiento($id, $dato){
		require_once ('../../appInit.php');
		$existe = $_TUCOACH->get_data("grw_tuc_p2p_comportamientos", " AND id_competencia = ".$id." AND nombre = '".($dato)."' ORDER BY id DESC ", 0);
		if($existe) {
