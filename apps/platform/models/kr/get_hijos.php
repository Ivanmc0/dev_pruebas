<?php require_once ('../../appInit.php');
	...
	if($_POST["id"] != "" && $_POST["nivel"] != "" && $_POST["proyecto"] != "" && $_POST["week"] != "" && $_POST["today"] != ""){
		...
		$datos = $_TUCOACH->get_data($tabla[$nivel]["tabla"], " AND ".$tabla[$nivel]["valor"]." = $id AND inactivo = 0 AND eliminado = 0 ".$ccodi, 1);
		$ccc = 1;
		if($datos){
			...
			foreach($datos AS $dato){
				...
				$responsable	= $_TUCOACH->get_data("zoom_users", " AND id = ".$dato["id_responsable"]." AND id_empresa = ".$_SESSION["COMPANY"]["id"]." AND inactivo = 0 AND eliminado = 0 ", 0);
				...
				$get_homeworks = 0;
				if($nivel < 5) $get_homeworks  = $_TUCOACH->get_data("grw_okr_tareas", " AND ".$tabla[$nivel+1]["valor"]." = ".$dato["id"]." AND id_empresa = ".$_SESSION["COMPANY"]["id"]." AND inactivo = 0 AND eliminado = 0 ORDER BY id_semana ASC, id ASC ", 1);
				...
				if($nivel == 5){
					if($star = $_OKR->get_task(" AND TASK.id = ".$dato["id"]." AND PROY.id = ".$dato["id_proyecto"]." AND EMP.id = ".$_SESSION["COMPANY"]["id"]."  ")){
						$star = $star[0];
						echo '
							<div class="">
								<div class="tab">
									<a class="tab20 p510 bN1 color999 bHover" href="'.$_SESSION["_DOMINION"].'tucoach/proyecto/'.$star["id_proyecto"].'/" data-toggle="tooltip" data-placement="top" title="'.($star["nombre_proyecto"]).'">
									...
									<a class="tab20 p510 bN2 color999 bHover" href="'.$_SESSION["_DOMINION"].'tucoach/proyecto/'.$star["id_proyecto"].'/objetivo/'.$star["id_objetivo"].'/" data-toggle="tooltip" data-placement="top" title="'.($star["nombre_objetivo"]).'">
									...
									<a class="tab20 p510 bN3 color999 bHover" href="'.$_SESSION["_DOMINION"].'tucoach/proyecto/'.$star["id_proyecto"].'/kr/'.$star["id_kr"].'/" data-toggle="tooltip" data-placement="top" title="'.($star["nombre_kr"]).'">
									...
									<a class="tab20 p510 bN4 color999 bHover" href="'.$_SESSION["_DOMINION"].'tucoach/proyecto/'.$star["id_proyecto"].'/accin/'.$star["id_accion"].'/" data-toggle="tooltip" data-placement="top" title="'.($star["nombre_accion"]).'">
									...
									<a class="tab20 p510 bN5 color999 bHover" href="'.$_SESSION["_DOMINION"].'tucoach/proyecto/'.$star["id_proyecto"].'/sprint/'.$star["id_sprint"].'/" data-toggle="tooltip" data-placement="top" title="'.($star["nombre_sprint"]).'">
				...
				echo '
						<div class="tab">
							<a href="'.$_SESSION["_DOMINION"].'proyecto/'. $_TUCOACH->url_seo((strtolower($tabla[$nivel]["nombre"]))).'/'.($dato["id"]).'/" class="tabIn bHover2">
								<div class="tab">
									<div class="tabIn p20">
				';
				...
				echo '
									<div class="tabIn pLR10" style="border-left:1px solid #eee; width:'.$va.';">
										...
										<div class="color666 t12">'. ucwords( strtolower( ( $responsable["cargo"] ? $responsable["cargo"] : '' ) )) .'</div>
									</div>
				';
				echo '
								</div>
				...
				if($nivel > 1) $vv = 2;
				if($nivel < 5){
					echo '<div onClick="Ion.get_hijos'.$vv.'('.($nivel+1).', '.$dato["id"].', '.$dato["id_proyecto"].', '.$_POST["week"].', '.$_POST["today"].');" class="nobel-'.($nivel+1).' nobel-'.($nivel+1).'-'.$dato["id"].' tabIn w60x aHover taC t20 colorMorado2 cP" style="border-left:1px solid #eee;"><i class="fas fa-level-down-alt"></i></div>';
					echo '<div onClick="Ion.close_hijos('.($nivel+1).', '.$dato["id"].', '.$dato["id_proyecto"].', '.$_POST["week"].', '.$_POST["today"].');" class="xnobel-'.($nivel+1).' xnobel-'.($nivel+1).'-'.$dato["id"].' tabIn w60x aHover taC t20 colorMorado2 cP" style="display:none;border-left:1px solid #eee;"><i class="fas fa-times"></i></div>';
				}
